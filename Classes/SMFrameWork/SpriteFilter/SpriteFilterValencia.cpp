...
#include "SpriteFilterValencia.h"
#include "FilterShaderString.h"
#include <cocos2d.h>


SpriteFilterValencia * SpriteFilterValencia::createWithTexture(cocos2d::Texture2D *texture)
{
    auto sprite = new (std::nothrow)SpriteFilterValencia();
...
SpriteFilterValencia::SpriteFilterValencia() :
_ext1(nullptr)
, _ext2(nullptr)
{
...
bool SpriteFilterValencia::initWithTexture(cocos2d::Texture2D *texture)
{
    if (!cocos2d::Sprite::initWithTexture(texture)) {
        return false;
    }
    
    auto addedSprite = cocos2d::Sprite::create("LookupImage/valenciaMap.png");
    _ext1 = addedSprite->getTexture();
    _ext1->retain();
    auto addedSprite2 = cocos2d::Sprite::create("LookupImage/valenciaGradientMap.png");
    _ext2 = addedSprite2->getTexture();
    _ext2->retain();
    
    std::string shaderKey = "ValenciaShader";
    
    auto program = cocos2d::GLProgramCache::getInstance()->getGLProgram(shaderKey);
    if (program==nullptr) {
        program = cocos2d::GLProgram::createWithByteArrays(FilterShaderString::FILTER_VS_BASE.c_str(), FilterShaderString::FILTER_FS_VALENCIA.c_str());
...
void SpriteFilterValencia::draw(cocos2d::Renderer *renderer, const cocos2d::Mat4 &transform, uint32_t flags)
{
    ...
    _customCommand.func = CC_CALLBACK_0(SpriteFilterValencia::onDraw, this, transform, flags);
    renderer->addCommand(&_customCommand);
}

void SpriteFilterValencia::onDraw(const cocos2d::Mat4 &transform, uint32_t flags)
{
