...
#include "SpriteFilterHudson.h"
#include "FilterShaderString.h"
#include <cocos2d.h>


SpriteFilterHudson * SpriteFilterHudson::createWithTexture(cocos2d::Texture2D *texture)
{
    auto sprite = new (std::nothrow)SpriteFilterHudson();
...
SpriteFilterHudson::SpriteFilterHudson() :
_ext1(nullptr)
, _ext2(nullptr)
, _ext3(nullptr)
{
...
bool SpriteFilterHudson::initWithTexture(cocos2d::Texture2D *texture)
{
    if (!cocos2d::Sprite::initWithTexture(texture)) {
        return false;
    }
    
    auto addedSprite = cocos2d::Sprite::create("LookupImage/hudsonBackground.png");
    ...
    _ext2->retain();
    auto addedSprite3 = cocos2d::Sprite::create("LookupImage/hudsonMap.png");
    _ext3 = addedSprite3->getTexture();
    _ext3->retain();
    
    std::string shaderKey = "HudsonShader";
    
    auto program = cocos2d::GLProgramCache::getInstance()->getGLProgram(shaderKey);
    if (program==nullptr) {
        program = cocos2d::GLProgram::createWithByteArrays(FilterShaderString::FILTER_VS_BASE.c_str(), FilterShaderString::FILTER_FS_HUDSON.c_str());
...
void SpriteFilterHudson::draw(cocos2d::Renderer *renderer, const cocos2d::Mat4 &transform, uint32_t flags)
{
    ...
    _customCommand.func = CC_CALLBACK_0(SpriteFilterHudson::onDraw, this, transform, flags);
    renderer->addCommand(&_customCommand);
}

void SpriteFilterHudson::onDraw(const cocos2d::Mat4 &transform, uint32_t flags)
{
