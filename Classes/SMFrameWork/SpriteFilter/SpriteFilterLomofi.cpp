...
#include "SpriteFilterLomofi.h"
#include "FilterShaderString.h"
#include <cocos2d.h>


SpriteFilterLomofi * SpriteFilterLomofi::createWithTexture(cocos2d::Texture2D *texture)
{
    auto sprite = new (std::nothrow)SpriteFilterLomofi();
...
SpriteFilterLomofi::SpriteFilterLomofi() :
_ext1(nullptr)
, _ext2(nullptr)
{
...
bool SpriteFilterLomofi::initWithTexture(cocos2d::Texture2D *texture)
{
    if (!cocos2d::Sprite::initWithTexture(texture)) {
        return false;
    }
    
    auto addedSprite = cocos2d::Sprite::create("LookupImage/lomoMap.png");
    ...
    std::string shaderKey = "LomofiShader";
    
    auto program = cocos2d::GLProgramCache::getInstance()->getGLProgram(shaderKey);
    if (program==nullptr) {
        program = cocos2d::GLProgram::createWithByteArrays(FilterShaderString::FILTER_VS_BASE.c_str(), FilterShaderString::FILTER_FS_LOMOFI.c_str());
...
void SpriteFilterLomofi::draw(cocos2d::Renderer *renderer, const cocos2d::Mat4 &transform, uint32_t flags)
{
    ...
    _customCommand.func = CC_CALLBACK_0(SpriteFilterLomofi::onDraw, this, transform, flags);
    renderer->addCommand(&_customCommand);
}

void SpriteFilterLomofi::onDraw(const cocos2d::Mat4 &transform, uint32_t flags)
{
