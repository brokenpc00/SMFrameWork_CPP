...
#include "SpriteFilterXproii.h"
#include "FilterShaderString.h"
#include <cocos2d.h>


SpriteFilterXproii * SpriteFilterXproii::createWithTexture(cocos2d::Texture2D *texture)
{
    auto sprite = new (std::nothrow)SpriteFilterXproii();
...
SpriteFilterXproii::SpriteFilterXproii() :
_ext1(nullptr)
, _ext2(nullptr)
{
...
bool SpriteFilterXproii::initWithTexture(cocos2d::Texture2D *texture)
{
    if (!cocos2d::Sprite::initWithTexture(texture)) {
        return false;
    }
    
    auto addedSprite = cocos2d::Sprite::create("LookupImage/xproMap.png");
    ...
    std::string shaderKey = "XproiiShader";
    
    auto program = cocos2d::GLProgramCache::getInstance()->getGLProgram(shaderKey);
    if (program==nullptr) {
        program = cocos2d::GLProgram::createWithByteArrays(FilterShaderString::FILTER_VS_BASE.c_str(), FilterShaderString::FILTER_FS_XPROII.c_str());
...
void SpriteFilterXproii::draw(cocos2d::Renderer *renderer, const cocos2d::Mat4 &transform, uint32_t flags)
{
    ...
    _customCommand.func = CC_CALLBACK_0(SpriteFilterXproii::onDraw, this, transform, flags);
    renderer->addCommand(&_customCommand);
}

void SpriteFilterXproii::onDraw(const cocos2d::Mat4 &transform, uint32_t flags)
{
