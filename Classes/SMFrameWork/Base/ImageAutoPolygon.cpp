...
_scaleFactor(0.0)
{
    CCASSERT(_image->getRenderFormat()==cocos2d::Texture2D::PixelFormat::RGBA8888, "unsupported format, image format must be RGBA8888");
    
    _data = _image->getData();
    _width = _image->getWidth();
    _height = _image->getHeight();
    _scaleFactor = cocos2d::Director::getInstance()->getContentScaleFactor();
}
...
std::vector<cocos2d::Vec2> ImageAutoPolygon::trace(const cocos2d::Rect &rect, const float& threshold)
{
    cocos2d::Vec2 first = findFirstNoneTransparentPixel(rect, threshold);
    return marchSquare(rect, first, threshold);
}

cocos2d::Vec2 ImageAutoPolygon::findFirstNoneTransparentPixel(const cocos2d::Rect &rect, const float &threshold)
{
    bool found = false;
    cocos2d::Vec2 i;
    ...
    CCASSERT(found==true, "image is all transparent");
    return i;
}


//2d/sprite-nodes/CAAutoPolygon

unsigned char ImageAutoPolygon::getAlphaByIndex(const unsigned int &i)
{
    return *(_data+i*4+3);
}

unsigned char ImageAutoPolygon::getAlphaByPos(const cocos2d::Vec2 &pos)
{
    return *(_data+((int)pos.y*_width+(int)pos.x)*4+3);
}

unsigned int ImageAutoPolygon::getSquareValue(const unsigned int &x, const unsigned int &y, const cocos2d::Rect &rect, const float &threshold)
{
    /*
    ...
    auto fixedRect = cocos2d::Rect(rect.origin, rect.size-cocos2d::Size(2,2));
    
    cocos2d::Vec2 tl = cocos2d::Vec2(x-1, y-1);
    sv += (fixedRect.containsPoint(tl) && getAlphaByPos(tl) > threshold)? 1 : 0;
    cocos2d::Vec2 tr = cocos2d::Vec2(x, y-1);
    sv += (fixedRect.containsPoint(tr) && getAlphaByPos(tr) > threshold)? 2 : 0;
    cocos2d::Vec2 bl = cocos2d::Vec2(x-1, y);
    sv += (fixedRect.containsPoint(bl) && getAlphaByPos(bl) > threshold)? 4 : 0;
    cocos2d::Vec2 br = cocos2d::Vec2(x, y);
    sv += (fixedRect.containsPoint(br) && getAlphaByPos(br) > threshold)? 8 : 0;
    CCASSERT(sv != 0 && sv != 15, "square value should not be 0, or 15");
    return sv;
}

std::vector<cocos2d::Vec2> ImageAutoPolygon::marchSquare(const cocos2d::Rect& rect, const cocos2d::Vec2& start, const float& threshold)
{
    int stepx = 0;
    int stepy = 0;
    int prevx = 0;
    ...
    do{
        ...
        else if(problem)
        {
            ...
            _points.push_back(cocos2d::Vec2((float)(curx- rect.origin.x) / _scaleFactor, (float)(rect.size.height - cury + rect.origin.y) / _scaleFactor));
        }
        else{
            _points.push_back(cocos2d::Vec2((float)(curx-rect.origin.x) / _scaleFactor, (float)(rect.size.height - cury + rect.origin.y) / _scaleFactor));
        }
...
float ImageAutoPolygon::perpendicularDistance(const cocos2d::Vec2& i, const cocos2d::Vec2& start, const cocos2d::Vec2& end)
{
    float res;
    float slope;
    float intercept;
...
std::vector<cocos2d::Vec2> ImageAutoPolygon::rdp(std::vector<cocos2d::Vec2> v, const float& optimization)
{
    if(v.size() < 3)
        return v;
    ...
    {
        std::vector<cocos2d::Vec2>::const_iterator begin = v.begin();
        std::vector<cocos2d::Vec2>::const_iterator end   = v.end();
        std::vector<cocos2d::Vec2> l1(begin, begin+index+1);
        std::vector<cocos2d::Vec2> l2(begin+index, end);
        
        std::vector<cocos2d::Vec2> r1 = rdp(l1, optimization);
        std::vector<cocos2d::Vec2> r2 = rdp(l2, optimization);
...
std::vector<cocos2d::Vec2> ImageAutoPolygon::reduce(const std::vector<cocos2d::Vec2>& points, const cocos2d::Rect& rect , const float& epsilon)
{
    ...
    float ep = cocos2d::clampf(epsilon, 0.0, maxEp/_scaleFactor/2);
    std::vector<cocos2d::Vec2> result = rdp(points, ep);
...
std::vector<cocos2d::Vec2> ImageAutoPolygon::expand(const std::vector<cocos2d::Vec2>& points, const cocos2d::Rect &rect, const float& epsilon)
{
    auto size = points.size();
    // if there are less than 3 points, then we have nothing
    if(size<3)
    ...
    {
        outPoints.push_back(cocos2d::Vec2(pt.X/PRECISION, pt.Y/PRECISION));
    }
    return outPoints;
}

cocos2d::TrianglesCommand::Triangles ImageAutoPolygon::triangulate(const std::vector<cocos2d::Vec2>& points)
{
    // if there are less than 3 points, then we can't triangulate
    if(points.size()<3)
    {
        return cocos2d::TrianglesCommand::Triangles();
    }
    ...
    for(const auto& ite : tris)
    {
        for(int i = 0; i < 3; ++i)
        {
            ...
            auto v3 = cocos2d::Vec3(p->x, p->y, 0);
            bool found = false;
            size_t j;
            size_t length = vdx;
            for(j = 0; j < length; j++)
            ...
            else
            {
                ...
                auto t2f = cocos2d::Tex2F(0,0); // don't worry about tex coords now, we calculate that later
    ...
    cocos2d::V3F_C4B_T2F* vertsBuf = new (std::nothrow) cocos2d::V3F_C4B_T2F[verts.size()];
    memcpy(vertsBuf, verts.data(), verts.size() * sizeof(cocos2d::V3F_C4B_T2F));
    ...
    cocos2d::TrianglesCommand::Triangles triangles = { vertsBuf, indicesBuf, static_cast<int>(verts.size()), static_cast<int>(indices.size()) };
    return triangles;
}

void ImageAutoPolygon::calculateUV(const cocos2d::Rect& rect, cocos2d::V3F_C4B_T2F* verts, const ssize_t& count)
{
    /*
...
cocos2d::Rect ImageAutoPolygon::getRealRect(const cocos2d::Rect& rect)
{
    cocos2d::Rect realRect = rect;
    //check rect to see if its zero
    if(realRect.equals(cocos2d::Rect::ZERO))
    {
        ...
        realRect = cocos2d::Rect(0,0, _width, _height);
    }
    else{
...
cocos2d::PolygonInfo* ImageAutoPolygon::generateTriangles(const cocos2d::Rect& rect, const float& epsilon, const float& threshold)
{
    cocos2d::Rect realRect = getRealRect(rect);
    ...
    cocos2d::PolygonInfo* ret = new cocos2d::PolygonInfo();
    ret->triangles = tri;
